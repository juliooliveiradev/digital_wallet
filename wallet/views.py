...
def home_view(request):
    if request.user.is_authenticated:
        return redirect('dashboard')  # Se o usuário estiver logado, redireciona para o dashboard
    else:
...
def add_balance_view(request):
    if request.method == 'POST':
        try:
            ...
            wallet = Wallet.objects.get(user=request.user)
            wallet.add_balance(amount)  # Usando o método add_balance para adicionar o valor
...
def transaction_history_view(request):
    ...
    if start_date and end_date:
        transactions = transactions.filter(date__range=[start_date + " 00:00", end_date + " 23:59"])
